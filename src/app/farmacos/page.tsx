...
// Map category name → icon (preferring snapshot icon)
const CAT_ICON: Record<string, string> = Object.fromEntries(
  catalog.categories.map((c) => [c.name, c.icon])
);
  ...
  return (
    <div className="wrap" style={{ paddingTop: "1.5rem", paddingBottom: "3rem" }}>
      ...
      <div style={{ marginBottom: "1.25rem" }}>
        ...
        >
          {catalog.drug_count} fármacos · {catalog.category_count} categorías ·
          v{catalog.version} · {catalog.last_updated}
        </p>
      </div>
        ...
        >
          ...
          >
            {selectedCategory === "__all__"
              ? `todos (${catalog.drug_count})`
              : `${CAT_ICON[selectedCategory] || ""} ${selectedCategory} (${results.length})`}
          </span>
        </div>
      )}
      ...
      {showCats && (
        <div className="fade-up">
          <div className="prompt mono" style={{ marginBottom: "0.5rem" }}>
            <b>$</b> ls categorías/
          </div>
          ...
          >
            [ ver todos — {catalog.drug_count} fármacos ]
          </button>
        </div>
      )}
      ...
      {query.trim() && results.length === 0 && (
        ...
        >
          <div
            style={{ fontSize: "2.5rem", opacity: 0.2, marginBottom: "0.75rem" }}
          >
            💀
          </div>
          <p
          ...
          <p
            ...
            style={{ color: "var(--text-3)", fontSize: "0.65rem" }}
          >
            Ese fármaco no está en nuestra base.
            <br />
...
function DrugCard({ drug }: { drug: Drug }) {
  const hasCalc = !!(drug.infusion && drug.infusion.length > 0);
  const presentation = hasCalc ? drug.infusion?.[0]?.ampule_presentation : null;
  return (
    <Link
    ...
    >
      ...
      {presentation && (
        <div style={{ display: "flex", gap: "0.35rem", marginTop: "0.5rem" }}>
          <span className="tag tag-muted mono">{presentation}</span>
        </div>
      )}
    </Link>
  );
