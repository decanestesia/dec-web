...
// El verificador en sí es gratis; la feature Pro es el uso ILIMITADO
// (Free = 3/día). El contador diario aún no está implementado — cuando lo
// esté, se lee aquí y se pasa a ProGateClient como condición extra. Por
// ahora, ProGateClient con el flag OFF deja pasar todo (cero cambio).
export default function InteraccionesClient({ isPro = false }: { isPro?: boolean }) {
  ...
  return (
    ...
    >
      ...
      {!loading && selected.length >= 2 && (
        // Gate de "interacciones ilimitadas". Flag OFF → pasa siempre.
        // El día que se active + exista el contador diario, un Free que
        // supere las 3/día verá el paywall aquí en vez de los resultados.
        <ProGateClient feature={PRO_FEATURES.UNLIMITED_INTERACTIONS} isPro={isPro}>
          <ResultsView
            results={results}
